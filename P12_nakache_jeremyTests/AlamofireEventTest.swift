...
class AlamofireEventTest: XCTestCase {
    ...
    override func setUp() {
        ...
        sut = EventsRepository(manager: manager)
    }
    ...
    func test_status_code200_returns_status_code200() throws {
        ...
        let expectedResult = EventsJsonStruct(get: "", parameters: ParametersEvents(fixture: ""), errors: [""], results: expectedResultCount, paging: PagingEvents(current: 1, total: 1), response: [])
        ...
        sut.getEvents(endPoint: "teams") { (result) in
            let event = try! result.get()
            XCTAssertEqual(event.results, expectedResultCount)
            expectation.fulfill()
        }
    ...
    func test_status_code400_returns_status_code400() throws {
        ...
        sut.getEvents(endPoint: "teams") { (result) in
            let event = result.isFailure
            XCTAssertEqual(event, true)
            expectation.fulfill()
        }
